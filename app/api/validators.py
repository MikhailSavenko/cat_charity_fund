...
async def check_change_full_amount(project_id, new_full_amount: int, session: AsyncSession):
    """Новая сумма сбора не ниже уже собранной"""
    full_amount = await project_crud.get_check_full_amount(project_id, new_full_amount, session)
    if not full_amount:
        raise HTTPException(
            status_code=400,
